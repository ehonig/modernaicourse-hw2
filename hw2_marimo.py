...
def _():
    ...
    This homework contains two main portions.   
    In part one, you will implement an extremely minimal automatic differentiation module.   
    This is the same technique that underlies PyTorch, and while you will not implement anything close to the complexity of a library like PyTorch, it will give you a basic understanding of the basic principles of the approach, giving you some insight into how the nuts and bolts of PyTorch do work under the hood.   
    In the second part, you will use the (built in) automatic differentiation tooling of PyTorch to train a simple linear model; note that for this assignment you won't do this in the "normal" PyTorch way of defining PyTorch Module subclasses, optimizer subclasses, and this sort of thing (that will be done in the next homework, on neural networks), but you will use the basic gradient descent approach to build a simple linear model.
    """)
    return
...
def _():
    ...
    At the core of automatic differentiation is a technique that builds a _compute graph_, which constructs a graph out of a series of functions applied to variables.   
    In our setting, we will implement this functionality with two simple classes: a `Variable` class that represents the variables will differentiate with respect to and a `Function` class that contains the logic to both implement the function itself and compute its gradient.
    """)
    return
...
def _():
    mo.md(r"""
    Let's discuss in some detail how the automatic differentiation works, which is best understood by an example.   
    Let's consider the following operation for two variables `x` and `y`:
    ```python
    (x*y + x**2)/y
    ```
    ...
    In this graph, the original variables and intermediate terms are represented as nodes, and the parents of a node represent the variables that were used to compute that term.   
    Although not depicted in the graph, each variable in the graph also stores a link to the function that created that variable (as a function of its parents).

    In our code, these computations graphs are modeled implicitly via the `Variable` class.   
    Specifically, the class contains the following items:
    ...
    In addition to the `Variable` class, there is also a `Function` class that creates variables in a fashion that builds the graph.   
    Specifically, the `__call__` method of the class implements the routine that constructs the graph. This lets you call a function like multiplication in the following manner:
    ```python
    Multiply()(x,y)
    ```
    ...
    1. The `.forward()` method actually computes the function.   
    For instance, the forward method of a `Multiply` class would multiply two numbers together, the forward pass of the `Log` class would take the log of a variable, etc.   
    As you see from the implementation above, this forward call is called by the `__call__()` class, but with additional code that constructions the graph.
    2. The `.backward()` function computes the _product_ of what's referred to as an "incoming derivative" term (this will correspond to the already-computed derivative of nodes later in the graph), and the _partial derivatives_ of this function.   
    In general, the arguments to the backward function will always be both this incoming derivative and the arguments to the original function.   
    For example, if we consider some function of two variables $f(x,y)$, and incoming derivative $g \in \mathbb{R}$, then the `.backward()` function would compute two separate product of partial derivatives, which are returned as a list.
...
def _():
    ...
    First let's show the implementation of a `Multiply` function:
    """)
    return
...
def _():
    ...
    The `.forward()` function simply implements this multiplication, `x*y`.   
    Furthermore, the partial derivatives of this particular function are given by
...
def _():
    ...
    Let's look at one more example, this time a function with a single argument, given by $f(x) = -x$.   
    In this case, the partial derivative is given by
...
def _():
    ...
    Implement the following `Function` classes to complete the implementation of the operators listed in the `Variable` class.   
    We are just providing the class definition itself: you'll need to define and implement `.forward()` and `.backward()` functions in each of these.   
    Remember that `.backward()` needs to always return a _list_ of products between the incoming derivative and each partial derivative, even if there is only a single argument.

    The one slightly-less straightforward implementation here is the `Power` class, which computes the operation `x**d`.   
    In this case, we won't actually enable differentiation with respect to the `d` variable (we certainly could, it's just a slightly more involved implementation function, so we don't do it in this assignment).   
    Thus, for this implementation, we'll store the `d` value in the class itself, and pass it to the `__init__()` operation of the function.   
    This is what's done in the `Variable` class above, i.e., whereas we call the `Multiply` class like the following:
    ```python
    Multiply()(x,y)
    ```
    (i.e., we initialize the class, then call it with the `x` and `y` arguments).   
    You would call `Power` via the following:
    ```python
    Power(d)(x)
    ```
    """)
...
class Power(Function):
    """
    Implements the power function between two variables f(x) = x^d.   
    Since the function does _not_ need to provide a derivative with respect to 
    the d argument, you should instead implement an __init__ function that 
    stores the d variable as a member, and use this in the forward pass.   
    The final usage of the class will then be what is done by our `Variable`
...
class Log(Function):
    """
    Implements the (natural) logarithm of a function f(x) = log(x).   
    You can use calls from the math package to implement this.
    """
...
class Exp(Function):
    """
    Implements the exponential (with base e) of x, f(x) = e^x.   
    You can use
...
def _():
    ...
    Given the functions defined above, now you'll implement the actual automatic differentiation pass that will extend the Variable class to compute gradients in a computation graph.   
    This is done with a `compute_gradients()` function called on the `Variable` instance [Note: in PyTorch this function is also called `.backward()`, but that can be a bit confusing since the method in `Function` is called the same thing, so I want to differentiate.]

    The basic algorithm is quite simple, and we'll outline it here, then explain some of the intuition behind it.   
    Note that this implementation makes very simplifying assumptions (for instance, it requires that variables not be used anywhere _except_ in the computation of the final output we are differentiating, or their `num_children` will never reach zero), and it doesn't, e.g., let you compute gradients multiple times (because it actively manipulates the `num_children` counters) but it nonetheless works for our simple cases.   
    ...
    Let's see how this works in a slightly different version of our example above (we'll ignore that last computation step, just to keep things simpler).   
    Note that going through all of this at first may not be needed, but it can be helpful to go through as you debug your implementation.
    ```python
    a = x*y
    b = x**2
    c = a + b
    ...
    ```
    Since the parents of `c` both have `num_children=0`, the function would then call `.compute_gradients()` recursively on each of these nodes.   
    Let's consider calling `a.compute_gradients()` first.   
    This would compute
    ```python
...
def compute_gradients(self):
    """
    Recursively compute derivatives in a computation graph.   
    This method iteratively computes the gradients for a node and 
    all its parents.
...
def _():
    mo.md(r"""
    In this second part of this lecture, you'll train a linear classifier using PyTorch.   
    While we showed an example of this process in class, we computed the gradients manually there (without much intuition on how that gradient was derived), and here you'll rely on PyTorch's automatic differentiation to actually compute the gradients.   
    Note that your implementation here will just be based upon functions, not PyTorch `Module` classes that are the more standardized way of building models in PyTorch (in the next assignment, we will use these module classes)
...
def _():
    ...
    Implement the following functions, which compute the cross entropy loss and the error between a set of predictions.   
    ...
    While you could use e.g. a for loop to compute cross entropy loss, this will be fairly inefficient later on.   
    Instead, you should use the fact that if you index a 2D tensor with two lists of indexes, it will select the elements corresponding to each of these indexes.   
    For instance given
...
def _():
    ...
    Finally, implement a minibatch version of the stochastic gradient descent method to optimize a linear classifier, using PyTorch.   
    Form a linear classifier specified by a matrix $W \in \mathbb{R}^{k \times n}$ (make sure to set `requires_grad=True` for this tensor, so you can compute gradients).
    ...
    Note that after you've taken this step, you'll want to zero out the gradients of $W$ (otherwise, new gradients will be _added_ on top of the older existing gradients in the `.grad` variable, which is not what you want).
    """)
    return
